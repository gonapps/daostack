...
const constraint = async function (method) {
  ...
  await controller.addGlobalConstraint(avatar.address,globalConstraints.address,0);
...
contract('Controller', function (accounts)  {
    it("mint reputation via controller", async () => {
        ...
        let tx =  await controller.mintReputation(avatar.address,amountToMint,accounts[0]);
    ...
    it("mint tokens via controller", async () => {
        ...
        let tx =  await controller.mintTokens(avatar.address,amountToMint,accounts[0]);
        assert.equal(tx.logs.length, 1);
    ...
    it("register schemes", async () => {
        controller = await  setup();
        let tx =  await controller.registerScheme(avatar.address,accounts[1], 0,0);
        assert.equal(tx.logs.length, 1);
    ...
    it("register schemes - check permissions for register new scheme", async () => {
      ...
      for(j = 0; j <= 15; j++ ){
        ...
        for(i = 0; i <= 15; i++ ){
          register = true;
          try {
                await controller.registerScheme(avatar.address,accounts[1],0,'0x'+uint32.toHex(i));
              } catch (ex) {
              ...
              if (register){
                await controller.unregisterScheme(avatar.address,accounts[1]);
                register= false;
              }
            }
        }
    ...
    it("register schemes - check permissions for updating existing scheme", async () => {
      ...
      controller = await  setup('0x0000000F');
       // scheme with permission 0x0000000F should be able to register scheme with permission 0x00000001
        let tx = await controller.registerScheme(avatar.address,accounts[0],0,'0x'+uint32.toHex(1));
        assert.equal(tx.logs.length, 1);
        assert.equal(tx.logs[0].event, "RegisterScheme");

        controller = await setup('0x00000001');
        try {
          await controller.registerScheme(avatar.address,accounts[0],0,'0x'+uint32.toHex(2));
    ...
    it("unregister schemes", async () => {
        controller = await  setup();
        let tx =  await controller.unregisterScheme(avatar.address,accounts[0]);
        assert.equal(tx.logs.length, 1);
        assert.equal(tx.logs[0].event, "UnregisterScheme");
    });
    it("unregister none registered scheme", async () => {
        controller = await  setup();
        let tx =  await controller.unregisterScheme(avatar.address,accounts[1]);
    ...
    it("unregister schemes - check permissions unregister scheme", async () => {
      ...
      for(i = 0; i <= 15; i++ ){
        //registered scheme has already permission to register(2)
        tx = await controller.registerScheme(avatar.address,registeredScheme,0,'0x'+uint32.toHex(i|3));
        assert.equal(tx.logs.length, 1);
        assert.equal(tx.logs[0].event, "RegisterScheme");
        for(j = 0; j <= 15; j++ ){
          tx = await controller.registerScheme(avatar.address,unregisteredScheme,0,'0x'+uint32.toHex(j));
          assert.equal(tx.logs.length, 1);
          assert.equal(tx.logs[0].event, "RegisterScheme");
          ...
          if (j&(~(i|3))) {
            ...
            try {
             await controller.unregisterScheme(avatar.address,unregisteredScheme,{ from: registeredScheme });
            ...
            tx = await controller.unregisterScheme(avatar.address,unregisteredScheme,{ from: registeredScheme });
            assert.equal(tx.logs.length, 1);
            assert.equal(tx.logs[0].event, "UnregisterScheme");
           }
         }
     ...
     it("unregister self", async () => {
      ...
      it("isSchemeRegistered ", async () => {
        ...
        controller = await  setup("0x00000000");
        isSchemeRegistered = await controller.isSchemeRegistered(avatar.address,accounts[1]);
        assert.equal(isSchemeRegistered, false);
        isSchemeRegistered = await controller.isSchemeRegistered(avatar.address,accounts[0]);
        assert.equal(isSchemeRegistered, true);
       });

      it("addGlobalConstraint ", async () => {
        controller = await  setup();
        var tx = await controller.addGlobalConstraint(avatar.address,accounts[1],0);
        assert.equal(await controller.isGlobalConstraintRegister(avatar.address,accounts[1]),true);
        assert.equal(tx.logs.length, 1);
       ...
       it("removeGlobalConstraint ", async () => {
         controller = await  setup();
         assert.equal(await controller.isGlobalConstraintRegister(avatar.address,accounts[1]),false);
         await controller.addGlobalConstraint(avatar.address,accounts[1],0);
         await controller.addGlobalConstraint(avatar.address,accounts[2],0);
         await controller.addGlobalConstraint(avatar.address,accounts[3],0);
         await controller.addGlobalConstraint(avatar.address,accounts[4],0);
         await controller.addGlobalConstraint(avatar.address,accounts[5],0);
         var tx = await controller.removeGlobalConstraint(avatar.address,accounts[3]);
         assert.equal(tx.logs.length, 1);
         assert.equal(tx.logs[0].event, "RemoveGlobalConstraint");
         assert.equal(await controller.isGlobalConstraintRegister(avatar.address,accounts[1]),true);
         assert.equal(await controller.isGlobalConstraintRegister(avatar.address,accounts[2]),true);
         assert.equal(await controller.isGlobalConstraintRegister(avatar.address,accounts[3]),false);
         assert.equal(await controller.isGlobalConstraintRegister(avatar.address,accounts[4]),true);
         assert.equal(await controller.isGlobalConstraintRegister(avatar.address,accounts[5]),true);
         assert.equal(await controller.globalConstraintsCount(avatar.address),4);
         await controller.removeGlobalConstraint(avatar.address,accounts[5]);
         assert.equal(await controller.isGlobalConstraintRegister(avatar.address,accounts[5]),false);
        ...
        it("upgrade controller ", async () => {
          ...
          var tx = await controller.upgradeController(avatar.address,accounts[1]);
          assert.equal(tx.logs.length, 1);
        ...
        it("upgrade controller check permission", async () => {
          ...
          await avatar.transferOwnership(controller.address);
          try{
            await controller.upgradeController(avatar.address,accounts[1]);
        ...
        it("generic call", async () => {
          controller = await  setup();
          await avatar.transferOwnership(controller.address);
          var tx = await controller.genericAction(avatar.address,[0]);
        ...
        it("sendEther", async () => {
          ...
          var tx = await controller.sendEther(avatar.address,web3.toWei('1', "ether"),otherAvatar.address);
          assert.equal(tx.logs.length, 1);
        ...
        it("externalTokenTransfer", async () => {
          ...
          await avatar.transferOwnership(controller.address);
          var tx = await controller.externalTokenTransfer(avatar.address,standardToken.address,accounts[1],50);
          assert.equal(tx.logs.length, 1);
        ...
        it("externalTokenTransferFrom & ExternalTokenIncreaseApproval", async () => {
          ...
          var standardToken = await StandardTokenMock.new(avatar.address, 100);
          await avatar.transferOwnership(controller.address);
          tx = await controller.externalTokenIncreaseApproval(avatar.address,standardToken.address,avatar.address,50);
          assert.equal(tx.logs.length, 1);
          assert.equal(tx.logs[0].event, "ExternalTokenIncreaseApproval");
          tx = await controller.externalTokenTransferFrom(avatar.address,standardToken.address,avatar.address,to,50);
          assert.equal(tx.logs.length, 1);
        ...
        it("externalTokenTransferFrom & externalTokenDecreaseApproval", async () => {
          ...
          await avatar.transferOwnership(controller.address);
          tx = await controller.externalTokenIncreaseApproval(avatar.address,standardToken.address,avatar.address,50);
          tx = await controller.externalTokenDecreaseApproval(avatar.address,standardToken.address,avatar.address,50);
          assert.equal(tx.logs.length, 1);
          assert.equal(tx.logs[0].event, "ExternalTokenDecreaseApproval");
          try{
             await controller.externalTokenTransferFrom(avatar.address,standardToken.address,avatar.address,to,50);
        ...
        it("globalConstraints mintReputation add & remove", async () => {
          ...
          await reputation.transferOwnership(controller.address);
          try {
          await controller.mintReputation(avatar.address,amountToMint,accounts[0]);
          assert(false,"mint reputation should fail due to the global constraint ");
          }
          catch(ex){
            helpers.assertVMException(ex);
          }
          await controller.removeGlobalConstraint(avatar.address,globalConstraints.address);
          ...
          it("globalConstraints mintTokens add & remove", async () => {
            ...
            await token.transferOwnership(controller.address);
            try {
            await controller.mintTokens(avatar.address,amountToMint,accounts[0]);
           ...
           it("globalConstraints register schemes add & remove", async () => {
              ...
              try {
              await controller.registerScheme(avatar.address,accounts[1], 0,0);
              ...
              it("globalConstraints unregister schemes add & remove", async () => {
                 ...
                 var globalConstraints = await constraint("registerScheme");
                 try {
                 await controller.unregisterScheme(avatar.address,accounts[0]);
                 ...
                 it("globalConstraints generic call  add & remove", async () => {
                    ...
                    try {
                    await controller.genericAction(avatar.address,[0]);
                    ...
                    it("globalConstraints sendEther  add & remove", async () => {
                       ...
                       try {
                        await controller.sendEther(avatar.address,web3.toWei('1', "ether"),otherAvatar.address);
                       ...
                       it("globalConstraints externalTokenTransfer  add & remove", async () => {
                          ...
                          try {
                           await controller.externalTokenTransfer(avatar.address,standardToken.address,accounts[1],50);
                          ...
                          it("globalConstraints externalTokenTransferFrom , externalTokenIncreaseApproval , externalTokenDecreaseApproval", async () => {
                             ...
                             try {
                              await controller.externalTokenIncreaseApproval(avatar.address,standardToken.address,avatar.address,50);
                             ...
                             try {
                              await controller.externalTokenDecreaseApproval(avatar.address,standardToken.address,avatar.address,50);
                             ...
                             await controller.externalTokenDecreaseApproval(avatar.address,standardToken.address,avatar.address,50);
                             try {
                              await await controller.externalTokenTransferFrom(avatar.address,standardToken.address,avatar.address,to,50);
                              assert(false,"externalTokenTransferFrom should fail due to decrease approval ");
                             }
                             catch(ex){
                               helpers.assertVMException(ex);
